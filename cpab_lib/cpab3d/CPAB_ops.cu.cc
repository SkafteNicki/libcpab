...
__device__ int findcellidx(const float* p, const int nx, const int ny, 
                            const int nz) {
    ...
    int p0 = mymin( nx-1 , max(0.0, point[0]*nx ) );
    int p1 = mymin( ny-1 , max(0.0, point[1]*ny ) );
    int p2 = mymin( nz-1 , max(0.0, point[2]*nz ) );
...
__global__ void calcTrans_kernel(const int nP, const int batch_size,
	...
	point[2] = points[point_index + 2*nP];
        ...
        int start_idx = batch_index * 12 * nx[0] * ny[0] * nz[0]; 
    
        // Iterate in nStepSolver
        int cellidx;
        for(int n = 0; n < nStepSolver[0]; n++){
            // Find cell idx
            cellidx = findcellidx(point, nx[0], ny[0], nx[0]);
        ...
        CUDA_AXIS_KERNEL_LOOP(batch_index, nthreads, x) {
            CUDA_AXIS_KERNEL_LOOP(point_index, nthreads, y) {
                CUDA_AXIS_KERNEL_LOOP(dim_index, nthreads, z) {
                    ...
                    // Define start index for the matrices belonging to this batch
                    // batch * num_elem * 4 triangles pr cell * cell in x * cell in y
                    int start_idx = batch_index * 12 * nx[0] * ny[0] * nx[0]; 
		    ...
		    p[2] = points[point_index + 2 * nP];
                    ...
                    for(int t=0; t<nStepSolver[0]; t++) {
                        // Get current cell
                        cellidx = findcellidx(p, nx[0], ny[0], nz[0]);
