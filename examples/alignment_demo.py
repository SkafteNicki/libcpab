...
if __name__ == "__main__":
    ...
    if args.alignment_type == 'sampling':
        theta_est = A.alignment_by_sampling(data, transformed_data, maxiter=args.maxiter)
    else:
        theta_est = A.alignment_by_gradient(data, transformed_data, maxiter=args.maxiter)
    
    # Lets see what we converged to
    trans_est = T.transform_data(data, theta_est, outsize=(350, 350))
    trans_est = trans_est.permute(0,2,3,1) if args.backend=='pytorch' else trans_est
